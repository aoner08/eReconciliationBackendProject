...
namespace Business.Concrete
{
	public class BaBsReconciliationManager : IBaBsReconciliationService
	{
		...
		[SecuredOperation("BaBsReconciliation.Add,Admin")]
		[CacheRemoveAspect("IBaBsReconciliationService.Get")]
		[TransactionScopeAspect]//hata olması durumunda tüm işlemleri iptal etsin
				...
				using (var reader = ExcelReaderFactory.CreateReader(stream))
				{
					while (reader.Read())
					{
						...
						if (code != "Cari Kodu" && code != null)
						{
							...
							string mounth = reader.GetString(2);
							string year = reader.GetString(3);
							string quantit = reader.GetString(4);
							string total = reader.GetString(5);
							string guid = Guid.NewGuid().ToString();
		...
		{
			return new SuccesDataResult<List<BaBsReconciliation>>(_baBsReconciliationDal.GetList(p => p.CompanyId == companyId)); //A
		}
		...
		{
			return new SuccesDataResult<List<BaBsReconciliationDto>>(_baBsReconciliationDal.GetAllDto(companyId)); //B
		}
		...
		public IResult SendReconciliationMail(BaBsReconciliationDto babsReconciliationDto)
		{
			...
			string link = "https://localhost:7096/api/BaBsReconciliations/GetByCode?code=" + babsReconciliationDto.Guid;
